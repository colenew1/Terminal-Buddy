...
try {
  ...
  // Move the same live process, then move it back before the existing recovery checks.
  await second.ev("window.__arrivals=[];window.buddy.workspace.onTransferArrive(items=>window.__arrivals.push(...items))")
  await first.ev(`document.querySelector('[data-move-session="${a}"]').click()`)
  await until(() => first.ev(`!!document.querySelector('[data-move-target="${secondId}"]')`), 'move destination list')
  await first.ev(`document.querySelector('[data-move-target="${secondId}"]').click()`)
  await until(() => second.ev("document.querySelectorAll('.tab[data-session-id]').length===2"), 'moved terminal arrives')
  await until(() => first.ev("document.querySelectorAll('.tab[data-session-id]').length===0"), 'source releases terminal')
  assert.equal(await second.ev(`window.__arrivals[0].session.id===${JSON.stringify(a)} && window.__arrivals[0].snapshot.data.includes('MOCK_READY:${apid}')`), true)
  process.kill(apid, 0)
  await write(first, a, 'OLD_OWNER')
  await write(second, a, 'AFTER_MOVE')
  await until(() => second.ev("window.__output.includes('INPUT:41465445525f4d4f5645')"), 'moved process accepts destination input')
  assert.equal(await first.ev("window.__output.includes('INPUT:41465445525f4d4f5645')"), false)
  assert.equal(await second.ev("window.__output.includes('INPUT:4f4c445f4f574e4552')"), false)
  assert.equal((await second.ev('window.buddy.workspace.get()')).sessions[1].title, 'Workspace Alpha')
  assert.equal((await first.ev('window.buddy.workspace.get()')).sessions.length, 0)
  pass('moving a tab preserves its process and screen while changing input ownership and recovery files')
  assert.match(await first.ev(`window.buddy.workspace.move(${JSON.stringify(a)},'primary').then(()=>'UNEXPECTED',e=>e.message)`), /owning workspace/)
  const dropPoint = await first.ev('({x:screenX+innerWidth/2,y:screenY+100})')
  await second.ev(`window.buddy.workspace.drag(${JSON.stringify(a)},${JSON.stringify(dropPoint)})`)
  await until(() => first.ev("!!document.querySelector('.transfer-hover')"), 'cross-window drop target')
  await second.ev(`window.buddy.workspace.drop(${JSON.stringify(a)},${JSON.stringify(dropPoint)})`)
  ...
  for (const client of [first, second]) {
    ...
    await ready(client)
    if (client === first) {
      const recoveredId = await sessionId(first)
      assert.match(await first.ev(`window.buddy.workspace.move(${JSON.stringify(recoveredId)},${JSON.stringify(secondId)}).then(()=>'UNEXPECTED',e=>e.message)`), /recovering sessions/)
      assert.equal(await first.ev("document.querySelectorAll('.tab[data-session-id]').length"), 1)
      assert.equal(await second.ev("!!document.querySelector('[data-restore-all]')"), true)
      pass('a pending recovery chooser rejects the entire move without losing either workspace')
    }
  }
  ...
  const thirdId = (await second.ev('window.buddy.app.windows()')).find(w => !w.current).id
  const third = await connect(workspacePage(thirdId))
  await chooseFresh(third); await ready(third)
  await second.ev("document.querySelector('[data-window-menu]').click()")
  await second.ev("document.querySelector('[data-combine-windows]').click()")
  await until(async () => (await pages()).length===1, 'combined windows close their empty sources')
  await until(() => second.ev("document.querySelectorAll('.tab[data-session-id]').length===2"), 'combined terminals present')
  await write(second, newB, 'AFTER_COMBINE')
  await until(() => second.ev("window.__output.includes('INPUT:41465445525f434f4d42494e45')"), 'process remains live after combine')
  assert.equal((await second.ev('window.buddy.workspace.get()')).sessions.length, 2)
  assert.deepEqual(JSON.parse(readFileSync(join(profile, 'windows.json'), 'utf8')), [secondId])
  pass('Combine all windows keeps processes live and saves one combined workspace')
