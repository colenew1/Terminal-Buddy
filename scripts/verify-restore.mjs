...
try {
  ...
  check('only a missing folder is unavailable; broken chat links stay reopenable',await ev("document.querySelectorAll('.restore-row.is-unavailable').length===1 && document.querySelectorAll('.restore-row.is-folder-only').length===3"))
  check('the reopen button stays clickable whenever any folder still exists',await ev("!document.querySelector('[data-restore-all]').disabled"))
  ...
  await until("document.querySelectorAll('.cell').length===6")
  ...
  check('unlinked chats reopen as plain folder terminals without launching an agent',saved().sessions.length===6 && saved().sessions.slice(3).every(s=>!s.resume && !s.agent && s.cwd.toLowerCase()===project.toLowerCase()))
  check('only the missing folder is retained for recovery',saved().unrestoredSessions.length===1 && saved().unrestoredSessions[0].title==='Missing folder')
